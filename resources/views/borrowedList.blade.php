                            ...
                            <tbody>
                                @foreach ($borrowed as $row)
                                    <tr id="filter">
                                        ...
                                        <td>
                                            {!! Form::open(['action' => ['DashBoardController@returnBook', $row->id], 'method' => 'POST']) !!}
                                                {{Form::hidden('_method', 'PUT')}}
                                                {{Form::submit('Return', ['class' => 'btn btn-primary'])}}
                                            {!! Form::close() !!}
                                        </td>
                                        <td>
                                            {!! Form::open(['action' => ['DashBoardController@lostBook', $row->id], 'method' => 'POST']) !!}
                                                {{Form::hidden('_method', 'PUT')}}
                                                {{Form::submit('Lost', ['class' => 'btn btn-primary'])}}
                                            {!! Form::close() !!}
                                        </td>
                                    </tr>
                                @endforeach
